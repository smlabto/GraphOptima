...
- **Manual Setup:** 
  - **Set Up [Apptainer](https://docs.alliancecan.ca/wiki/Apptainer) Virtual Container:** For detailed steps, refer
     to [Setting up cuGraph layout generators on Compute Canada's Graham Cluster Using Singularity.md](https://github.com/smlabto/GraphOptima/blob/master/Setting%20up%20cuGraph%20layout%20generators%20on%20Compute%20Canada's%20Graham%20Cluster%20Using%20Singularity.md) and follow steps
     1–4.
  ...
  -  **Generate Sample Graphs (Optional):** By default, the GraphOptima will optimize a example price network stored under the `input_graph` folder as `.graphml`. However, if you want to generate more graphs After setting up the singularity instance, generate test graph for test optimizations. 
     Following instructions in [Use GraphML in GraphOptima.md](https://github.com/smlabto/GraphOptima/blob/master/Sample%20Graph%20Generation%20Tutorial.md).
  ...
  - **Set Up GLAM Layout Evaluator:** Finally, set up the GLAM layout evaluator. See steps 1–4
     in [Setting up GLAM layout evaluators on Compute Canada.md](https://github.com/smlabto/GraphOptima/blob/master/Setting%20up%20GLAM%20layout%20evaluators%20on%20Compute%20Canada.md). After the installation, ensure that the `GLAM` executable
...
1. **Configure JSON File:** Configure the `config.json` file following the instructions
   in [How to set up the config file.md](https://github.com/smlabto/GraphOptima/blob/master/Setting%20up%20GLAM%20layout%20evaluators%20on%20Compute%20Canada.md).
...
- After the `.mat` file is generated, follows [Collecting, Extracting, and Interpreting Optimization Results using Python and MATLAB.md](https://github.com/smlabto/GraphOptima/blob/master/Collecting%2C%20Extracting%2C%20and%20Interpreting%20Optimization%20Results%20using%20Python%20and%20MATLAB.md) to visualize the
...
in [Using cuGraph, Singularity, and Jupyter Notebook to Reproduce and Visualize Layouts.md](https://github.com/smlabto/GraphOptima/blob/master/Using%20cuGraph%2C%20Singularity%2C%20and%20Jupyter%20Notebook%20to%20Reproduce%20and%20Visualize%20Layouts.md).
